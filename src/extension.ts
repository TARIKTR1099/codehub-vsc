...
const T: Record<string, Record<string, string>> = {
  ...
  addProfile: _("Profil Ekle", "Add Profile"), noProfiles: _("Henüz profil yok. + ile ekleyin.", "No profiles yet. Tap +."),
...
async function pickProf(title: string, sk: string) {
  ...
  const items = [
    { label: `$(chevron-up) ${t("first")} ${cur==="first"?" \u25C8":""}`, id: "first" },
    ...getP().map((p) => ({ label: `$(${p.icon||"terminal"}) ${p.name} ${cur===p.id?"\u25C8":""}`, description: `${buildCmd(p)} \u2022 ${p.shell}`, id: p.id })),
  ];
  const pick = await vscode.window.showQuickPick(items, { placeHolder: title, matchOnDescription: true });
  if (pick) c().update(sk, pick.id, vscode.ConfigurationTarget.Global);
}
...
async function expProf() {
  ...
  if (!uri) return;
  try { vscode.workspace.fs.writeFile(uri, Buffer.from(JSON.stringify(getP(), null, 2), "utf-8")); vscode.window.showInformationMessage(t("exported")); } catch { vscode.window.showErrorMessage("Export failed"); }
}
...
async function pickIcon(existing?: string): Promise<string | undefined> {
  const ch = await vscode.window.showQuickPick([
    { label: `$(close) Devam et (Enter)`, id: "none" },
  ...
  const icons = ["terminal","terminal-powershell","terminal-cmd","terminal-bash","terminal-linux","browser","comment-discussion","git-branch","github","symbol-variable","symbol-ruler","symbol-key","server","server-process","database","cloud","tools","beaker","light-bulb","flame","star","rocket","zap","sync","play","debug","package","code","sparkle","wand","shield","hubot","organization","link","extensions","globe","lock","key","pulse","graph","book","note","bell","clock"];
...
async function profDlg(existing?: Profile): Promise<Profile | undefined> {
  ...
  const cwd = await vscode.window.showInputBox({ prompt: t("dir") + " (opsiyonel)", value: existing?.cwd || "", placeHolder: "C:\\Projects\\my-app" }); if (cwd === undefined) return;
...
class SidePanel implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(vw: vscode.WebviewView) {
    ...
    vw.webview.onDidReceiveMessage(async (m) => {
      ...
      if (m.type === "delete") { const p = findP(m.id); if (!p) return; const list = getP().filter((x) => x.id !== m.id); setP(list); if (c().get<string>("defaultProfile","") === m.id) c().update("defaultProfile","",vscode.ConfigurationTarget.Global); this.render(); vscode.window.showInformationMessage(`${t("deleted")}: ${p.name}`); return; }
      if (m.type === "setDefault") { c().update("defaultProfile",m.id,vscode.ConfigurationTarget.Global); this.render(); const p = findP(m.id); if (p) vscode.window.showInformationMessage(`${p.name} \u2605 ${t("defaultSet")}`); return; }
  ...
  private html(): string {
    ...
    const hasDef = getP().some((p)=>p.id===def);
...
body{padding:14px;font-family:var(--vscode-font-family);font-size:var(--vscode-font-size);color:var(--vscode-foreground);line-height:1.4}
